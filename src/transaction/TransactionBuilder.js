...
import { Signature, PublicKey, hash } from "dxpdex-ecc";
import { getConfig, database, network } from "dxpdex-api";
import { ops, ChainTypes } from "dxpdex-serializer";
import { Buffer } from "safe-buffer";

var head_block_time_string, dxpcore_min_review;
...
class TransactionBuilder {
  ...
  /**
        ...
        @arg {array<string>} [signer_pubkeys = null] - Optional ["GPHAbc9Def0...", ...].  These are additional signing keys.  Some balance claims require propritary address formats, the blockproducer node can't tell us which ones are needed so they must be passed in.  If the blockproducer node can figure out a signing key (mostly all other transactions), it should not be passed in here.

        @arg {boolean} [broadcast = false]
    */
  process_transaction(cwallet, signer_pubkeys = null, broadcast = false) {
    let wallet_object = cwallet.wallet.wallet_object;
    if (getConfig().chainId !== wallet_object.get("chain_id"))
  ...
  finalize() {
    return new Promise((resolve, reject) => {
      ...
      resolve(
        database.getObjects(["2.1.0"]).then(r => {
          head_block_time_string = r[0].time;
          if (this.expiration === 0)
            this.expiration = base_expiration_sec() + getConfig().expireInSecs;
  ...
  get_type_operation(name, operation) {
    ...
    if (name === "proposal_create") {
      /*
       * Proposals involving the dxpcore account require a review
      ...
      operation.proposed_ops.forEach(op => {
        ...
        switch (op.op[0]) {
          ...
          case 12: // asset_update_smarttoken
          ...
          case 31: // dxpcore_member_update_global_parameters
            requiresReview = true;
      ...
      operation.expiration_time ||
        (operation.expiration_time =
          base_expiration_sec() + getConfig().expireInSecsProposal);
      if (requiresReview) {
        operation.review_period_seconds =
          extraReview +
          Math.max(
            dxpcore_min_review,
            24 * 60 * 60 || getConfig().reviewInSecsDxpcore
          );
        /*
  ...
  update_head_block() {
    return database.getObjects(["2.0.0", "2.1.0"]).then(function (res) {
      let [g, r] = res;
      head_block_time_string = r[0].time;
      dxpcore_min_review = g[0].parameters.dxpcore_proposal_review_period;
    });
  }
  ...
  /** optional: the fees can be obtained from the blockproducer node */
  set_required_fees(asset_id, removeDuplicates) {
    ...
    promises.push(
      Promise.all(
        feeAssets.map(id => {
          return database.getRequiredFees(operations, id);
        })
      ).catch(err => {
    ...
    if (feeAssets.length > 1 || feeAssets[0] !== "1.3.0") {
      ...
      promises.push(database.getRequiredFees(operations, "1.3.0"));
      promises.push(database.getObjects(dynamicObjectIds));
    }
  ...
  sign(chain_id = getConfig().chainId) {
    if (!this.tr_buffer) {
      throw new Error("not finalized");
    }
    if (this.signed) {
