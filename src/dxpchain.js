...
import { PrivateKey, PublicKey, Aes, key } from "dxpdex-ecc";
import { setAddressPrefix } from "dxpdex-ecc";
import {
  connect,
  disconnect,
  database,
...
class DxpChain {
  ...
  static node = "ws://59.92.106.126:11011";
  ...
  static async connect(
    node = DxpChain.node,
    autoreconnect = DxpChain.autoreconnect
  ) {
    if (DxpChain.connectPromise) return DxpChain.connectPromise;

    await (DxpChain.connectPromise = DxpChain.reconnect(node, autoreconnect));
  ...
  static disconnect() {
    DxpChain.connectPromise = undefined;
    DxpChain.autoreconnect = false;
    disconnect();
  }

  static async reconnect(node, autoreconnect) {
    DxpChain.chain = await connect(node, undefined, autoreconnect);
    setAddressPrefix(DxpChain.chain.addressPrefix);
    DxpChain.node = node;

    return DxpChain.chain;
  }
  ...
  static async login(
    accountName,
    password,
    feeSymbol = DxpChain.chain.coreToken
  ) {
    let acc = await DxpChain.accounts[accountName],
    ...
    let account = new DxpChain(accountName, activeKey.toWif(), feeSymbol);
  ...
  static async tradeHistory(
    ...
    return history.getMarketHistory(
      (await DxpChain.assets[quoteSymbol]).id,
      (await DxpChain.assets[baseSymbol]).id,
      bucketSeconds,
  ...
  constructor(accountName, activeKey, feeSymbol = DxpChain.chain.coreToken) {
    ...
    this.initPromise = Promise.all([
      DxpChain.accounts[accountName],
      DxpChain.assets[feeSymbol],
    ]).then(params => {
  ...
  setFeeAsset = async feeSymbol => {
    ...
    this.feeAsset = await DxpChain.assets[feeSymbol];
  };
  ...
  balances = async (...args) => {
    ...
    let assets = await Promise.all(
      args.map(async asset => (await DxpChain.assets[asset]).id)
    );
    let balances = await database.getAccountBalances(this.account.id, assets);
    return Promise.all(
      balances.map(balance => DxpChain.assets.fromParam(balance))
    );
  };
  ...
  limitOrderCreateOperation = async (
    ...
    let buyAsset = await DxpChain.assets[buySymbol],
      sellAsset = await DxpChain.assets[sellSymbol];
  ...
  buyOperation = async (
    ...
    let buyAsset = await DxpChain.assets[buySymbol],
      baseAsset = await DxpChain.assets[baseSymbol],
  ...
  sellOperation = async (
    ...
    let sellAsset = await DxpChain.assets[sellSymbol],
      baseAsset = await DxpChain.assets[baseSymbol],
  ...
  memo = async (toName, message) => {
    ...
    let nonce = Date.now().toString(), //TransactionHelper.unique_nonce_uint64(),
      to = (await DxpChain.accounts[toName]).options.memo_key;
  ...
  transferOperation = async (toName, assetSymbol, amount, memo) => {
    ...
    let asset = await DxpChain.assets[assetSymbol],
    ...
    let params = {
      ...
      to: (await DxpChain.accounts[toName]).id,
  ...
  assetIssueOperation = async (toName, assetSymbol, amount, memo) => {
    ...
    let params = {
      ...
      issue_to_account: (await DxpChain.accounts[toName]).id,
    };
...
Event.init(DxpChain.connect);

export default DxpChain;
